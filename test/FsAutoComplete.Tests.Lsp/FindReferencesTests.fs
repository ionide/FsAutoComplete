...
let tests state = testList "Find All References tests" [
  ...
  testCaseAsync "Can find references for foo identifier in script" (async {
    ...
    | Ok (Some references) ->
      Expect.hasLength references 1 "Should have a reference for the definition only"
      let reference = references.[0]
      Expect.stringEnds reference.Uri (Path.GetFileName scriptPath) "should point to the same script"
      Expect.equal reference.Range { Start = { Line = 0; Character = 4 }; End = { Line = 0; Character = 7 } } "should point to the definition of `foo`"
  })
]
