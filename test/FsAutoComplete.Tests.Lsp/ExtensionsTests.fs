...
let fakeInteropTests state =
  let server =
    async {
      let folderPath = Path.Combine(__SOURCE_DIRECTORY__, "TestCases", "FakeInterop")
      let! (server, events) = serverInitialize folderPath defaultConfigDto state
      let buildScript = "build.fsx"
      do! waitForWorkspaceFinishedParsing events
      return server, events, folderPath, buildScript
    } |> Async.Cache

  testList "fake integration" [
    testCaseAsync "can typecheck a fake script including uses of paket-delivered types" (async {
      let! (server, events, rootPath, buildScript) = server
      do! server.TextDocumentDidOpen { TextDocument = loadDocument (Path.Combine(rootPath, buildScript)) }

      match! waitForParseResultsForFile buildScript events with
      | Ok () ->
        () // all good, no parsing/checking errors
      | Core.Result.Error errors ->
        failwithf "Errors while parsing script %s: %A" (Path.Combine(rootPath, buildScript)) errors
    })
  ]
