...
module OpenTelemetry =
  ...
  type Activity with
    member inline x.SetSource
      ...
      if not (isNull x) then
        ...
        if x.GetTagItem "code.namespace" = null then
          x.SetTag("code.namespace", name_space) |> ignore

        if x.GetTagItem "code.function" = null then
          x.SetTag("code.function", defaultArg memberName "") |> ignore

        if x.GetTagItem "code.filepath" = null then
          x.SetTag("code.filepath", defaultArg path "") |> ignore

        if x.GetTagItem "code.lineno" = null then
          x.SetTag("code.lineno", defaultArg line 0) |> ignore
  ...
  module internal Activity =
    ...
    let inline setExn (e: exn) (span: Activity) =
      if isNotNull span |> not then
        ...
        ActivityEvent("exception", tags = tags) |> span.AddEvent |> ignore
  ...
  module TestResult =
    let ofException (e: Exception) : TestResult =
      ...
      | :? FailedException as e -> Failed("\n" + e.Message)
      | :? IgnoreException as e -> Ignored e.Message
