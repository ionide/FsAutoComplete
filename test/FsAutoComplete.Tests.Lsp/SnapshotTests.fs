...
module Dotnet =
  let restore (projectPath: FileInfo) =
    async {
      let! cmd = Helpers.runProcess projectPath.Directory.FullName "dotnet" "restore"
      Helpers.expectExitCodeZero cmd
    }

  let restoreAll (projectPaths: FileInfo seq) =
    async { do! projectPaths |> Seq.map (fun p -> restore p) |> Async.Sequential |> Async.Ignore }

  let addPackage (projectPath: FileInfo) (packageName: string) (version: string) =
    async {
      let! cmd =
        Helpers.runProcess projectPath.Directory.FullName "dotnet" $"add package {packageName} --version {version}"

      Helpers.expectExitCodeZero cmd
    }

  let removePackage (projectPath: FileInfo) (packageName: string) =
    async {
...
module Projects =
  module Simple =
    let simpleProjectDir =
      DirectoryInfo(__SOURCE_DIRECTORY__ </> "TestCases/ProjectSnapshot/SimpleProject")

    let simpleProject = "SimpleProject.fsproj"
    let projects (srcDir: DirectoryInfo) = [ FileInfo(srcDir.FullName </> simpleProject) ]

  module MultiProjectScenario1 =
    let multiProjectScenario1Dir =
      DirectoryInfo(__SOURCE_DIRECTORY__ </> "TestCases/ProjectSnapshot/MultiProjectScenario1")
    ...
    let projects (srcDir: DirectoryInfo) = [ Console1.projectIn srcDir; Library1.projectIn srcDir ]
...
let createProjectA (projects: FileInfo seq) (loader: IWorkspaceLoader) onLoadCallback =
  ...
  let loadedProjectsA =
    ...
    |> AVal.map (fun kvp ->
      ...
      let loaded =
        projects |> List.map (fun p -> p.FullName) |> loader.LoadProjects |> Seq.cache

      onLoadCallback ()
      loaded |> Seq.map (fun l -> normalizePath l.ProjectFileName, l)
    // projects
...
let snapshotTests loaders toolsPath =
  ...
  testList
    ...
    [ for (loaderName, workspaceLoaderFactory) in loaders do
        ...
        <| testList
          $"{loaderName}"
          [ testCaseAsync "Simple Project Load"
            <| async {
              ...
              Expect.equal
                1
                loadedCalls
                "Load Projects should only get called once after doing nothing that should trigger a reload"
            }

            testCaseAsync "Adding nuget package should cause project load"
            <| async {
              let (loader: IWorkspaceLoader) = workspaceLoaderFactory toolsPath
              let srcDir = Projects.Simple.simpleProjectDir
              use dDir = Helpers.DisposableDirectory.From srcDir
            ...
            testCaseAsync "Create snapshot"
            <| async {
              let (loader: IWorkspaceLoader) = workspaceLoaderFactory toolsPath
              let srcDir = Projects.Simple.simpleProjectDir

              let sourceTextFactory: FsAutoComplete.ISourceTextFactory =
              ...
              Expect.equal
                ...
                (UMX.untag project)
                "Snapshot should have the same project file name as the project"

              Expect.equal
                (Seq.length snapshot.SourceFiles)
                3
                "Snapshot should have the same number of source files as the project"

              Expect.equal
                (Seq.length snapshot.ReferencedProjects)
                0
                "Snapshot should have the same number of referenced projects as the project"
            }


            testCaseAsync "Cached Adaptive Snapshot - MultiProject - Updating nothing shouldn't cause recalculation"
            <| asyncEx {
              let (loader: IWorkspaceLoader) = workspaceLoaderFactory toolsPath
              ...
              use dDir =
                Helpers.DisposableDirectory.From Projects.MultiProjectScenario1.multiProjectScenario1Dir
              ...
              let ls1 =
                snapshotBefore
                |> HashMap.find (
                  normalizePath (Projects.MultiProjectScenario1.Library1.projectIn dDir.DirectoryInfo).FullName
                )
              ...
              let cs1 =
                snapshotBefore
                |> HashMap.find (
                  normalizePath (Projects.MultiProjectScenario1.Console1.projectIn dDir.DirectoryInfo).FullName
                )
            ...
            testCaseAsync
              "Cached Adaptive Snapshot - MultiProject - Updating Source file in Console recreates Console snapshot"
            <| asyncEx {
              let (loader: IWorkspaceLoader) = workspaceLoaderFactory toolsPath
              ...
              let consoleFile =
                Projects.MultiProjectScenario1.Console1.programFileIn dDir.DirectoryInfo
              ...
              Expect.equal
                cs1.ReferencedProjects.Length
                cs2.ReferencedProjects.Length
                "Referenced projects length should be the same"

              Expect.equal cs1.ReferencedProjects.Length 1 "Referenced projects length should be 1"

              let refLib1 =
                cs1.ReferencedProjects
                |> Seq.tryPick (fun x ->
                  match x with
                  | FSharpReferencedProjectSnapshot.FSharpReference(_, x) -> Some x
                  | _ -> None)
                |> Option.get
              ...
              Expect.equal
                refLib1
                refLib2
                "Referenced library in both snapshots should be the same as library did not change in this test"

              Expect.notEqual cs1.Stamp cs2.Stamp "Stamp should not be the same"
            }

            testCaseAsync
              "Cached Adaptive Snapshot - MultiProject - Updating Source file in Library recreates Library and Console snapshot"
            <| asyncEx {
              let (loader: IWorkspaceLoader) = workspaceLoaderFactory toolsPath
              ...
              let libraryFile =
                Projects.MultiProjectScenario1.Library1.libraryFileIn dDir.DirectoryInfo
              ...
              Expect.equal
                libBefore.SourceFiles.Length
                libAfter.SourceFiles.Length
                "Source files length should be the same"

              let ls1File =
                libBefore.SourceFiles
                |> Seq.find (fun x -> x.FileName = normalizeUntag libraryFile.FullName)

              let ls2File =
                libAfter.SourceFiles
                |> Seq.find (fun x -> x.FileName = normalizeUntag libraryFile.FullName)

              Expect.notEqual ls1File.Version ls2File.Version "Library source file version should not be the same"

              Expect.equal
                libBefore.ReferencedProjects.Length
                libAfter.ReferencedProjects.Length
                "Referenced projects length should be the same"
              ...
              Expect.equal
                consoleBefore.ProjectFileName
                consoleAfter.ProjectFileName
                "Project file name should be the same"
              ...
              Expect.equal
                consoleBefore.SourceFiles.Length
                consoleAfter.SourceFiles.Length
                "Source files length should be the same"
              ...
              let cs1File =
                consoleBefore.SourceFiles
                |> Seq.find (fun x -> x.FileName = normalizeUntag consoleFile.FullName)

              let cs2File =
                consoleAfter.SourceFiles
                |> Seq.find (fun x -> x.FileName = normalizeUntag consoleFile.FullName)

              Expect.equal cs1File.Version cs2File.Version "Console source file version should be the same"

              Expect.equal
                consoleBefore.ReferencedProjects.Length
                consoleAfter.ReferencedProjects.Length
                "Referenced projects length should be the same"

              Expect.equal consoleBefore.ReferencedProjects.Length 1 "Referenced projects length should be 1"

              let refLib1 =
                consoleBefore.ReferencedProjects
                |> Seq.tryPick (fun x ->
                  match x with
                  | FSharpReferencedProjectSnapshot.FSharpReference(_, x) -> Some x
                  | _ -> None)
              ...
              let refLib2 =
                consoleAfter.ReferencedProjects
                |> Seq.tryPick (fun x ->
                  match x with
                  | FSharpReferencedProjectSnapshot.FSharpReference(_, x) -> Some x
                  | _ -> None)
              ...
              Expect.notEqual
                refLib1
                refLib2
                "Referenced library from different snapshot should not be the same as library source file changed"
