...
module Cursor =
    /// 0-based
    val inline private pos: line: int -> column: int -> Position
    ...
    val beforeIndex: i: int -> text: string -> Position
    ...
    val tryExtractIndex: text: string -> (int * string) option
    /// `tryExtractIndex`, but fails when there's no cursor
    val assertExtractIndex: (string -> int * string)
    ...
    val tryIndexOf: pos: Position -> text: string -> Result<int, string>
    /// `tryIndexOf`, but fails when position is invalid
    val assertIndexOf: pos: Position -> (string -> int)
    ...
    val afterEdits: edits: TextEdit list -> pos: Position -> Position
...
module TextEdits =
    ...
    val tryFindError: edits: TextEdit list -> string option
    /// Sorts edits by range (`Start`).
    /// Order is preserved for edits with same `Start`.
    val sortByRange: edits: TextEdit list -> TextEdit list
    /// Applies the passed edits from last to first (sorted by range)
    val apply: edits: TextEdit list -> text: string -> Result<string, string>
    /// `tryFindError` before `apply`
    val applyWithErrorCheck: edits: TextEdit list -> text: string -> Result<string, string>
...
module WorkspaceEdit =
    ...
    val tryExtractTextEditsInSingleFile:
        textDocument: VersionedTextDocumentIdentifier -> workspaceEdit: WorkspaceEdit -> Result<TextEdit list, string>
