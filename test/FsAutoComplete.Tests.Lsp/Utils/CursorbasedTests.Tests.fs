...
module private CodeFix =
  ...
  let private checkNotApplicableTests server =
    testList
      (nameof CodeFix.checkNotApplicable)
      [ let expectFailCheckNotApplicable server beforeWithCursor chooseFix =
        ...
        <| CodeFix.checkNotApplicable
          ...
          "let a$0 = 42"
          ignore
          (CodeFix.withTitle "Add missing '=' to type definition" >> expectLength 0)
        ...
        <| expectFailCheckNotApplicable server "let a$0 = 42" ((CodeFix.withTitle "Replace with _") >> expectLength 1)

        testCaseAsync "not not applicable when multiple code actions"
        <| expectFailCheckNotApplicable server "let a$0 = 42" (expectAtLeast 2) ]

  let private checkApplicableTests server =
    testList
      (nameof CodeFix.checkApplicable)
      [ let expectFailCheckApplicable server beforeWithCursor chooseFix =
          CodeFix.checkApplicable server beforeWithCursor ignore chooseFix
          |> Expect.failure

        testCaseAsync "applicable when one code action"
        <| CodeFix.checkApplicable server "let a$0 = 42" ignore ((CodeFix.withTitle "Replace with _") >> expectLength 1)
        ...
        <| expectFailCheckApplicable server "let a$0 = 42" (expectAtLeast 2) ]

  let private checkTests server =
    testList
      (nameof CodeFix.check)
      [ let expectFailCheck server beforeWithCursor chooseFix expected =
        ...
        <| CodeFix.check
          ...
          "let a$0 = 42"
          ignore
          ((CodeFix.withTitle "Replace with _") >> expectLength 1)
          "let _ = 42"

        testCaseAsync "fails when unexpected output"
        <| expectFailCheck server "let a$0 = 42" ((CodeFix.withTitle "Replace with _") >> expectLength 1) "let b = 42"
        ...
        <| expectFailCheck server "let a$0 = 42" (expectAtLeast 2) "let _ = 42" ]
...
let tests state = testList (nameof Utils.CursorbasedTests) [ CodeFix.tests state ]
