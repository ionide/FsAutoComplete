...
let private posIsInLhsOfClause (pos: pos) (clause: SynMatchClause) =
  ...
  | Clause (_, Some guardExpr, _, patternRange, _) -> rangeContainsPos (unionRanges guardExpr.Range patternRange) pos
  ...
  | last :: _ ->
      ...
      let possibleBarLocationRange =
        ...
        if patMatchExpr.MatchWithOrFunctionRange.EndLine = firstClauseOnLastLine.Range.StartLine then
          unionRanges patMatchExpr.MatchWithOrFunctionRange.EndRange firstClauseOnLastLine.Range.StartRange
        else
...
let checkThatPatternMatchExprEndsWithCompleteClause (expr: PatternMatchExpr) =
  ...
  | lastClause :: _ ->
      ...
      | Clause (_, _, SynExpr.Const (SynConst.Unit, unitRange), _, _) ->
          ...
          rhsExprExists
          && not (synExprContainsError expr.Expr)
      ...
      let fieldNames =
        [| for field in case.UnionCaseFields ->
             if String.IsNullOrEmpty(field.Name)
                || isUnnamedUnionCaseField field then
               "_"
             else
