...
type Commands(serialize: Serializer, backgroundServiceEnabled, toolsPath) =
  let checker =
    FSharpCompilerServiceChecker(backgroundServiceEnabled)

  let state = State.Initial toolsPath
  let fileParsed = Event<FSharpParseFileResults>()

  let fileChecked =
    Event<ParseAndCheckResults * string * int>()
  ...
  let mutable linterConfiguration: FSharpLint.Application.Lint.ConfigurationParam =
    FSharpLint.Application.Lint.ConfigurationParam.Default

  let mutable lastVersionChecked = -1
  let mutable lastCheckResult: ParseAndCheckResults option = None

  let mutable analyzerHandler: ((string * string [] * FSharp.Compiler.SyntaxTree.ParsedInput * FSharpImplementationFileContents * FSharpEntity list * (bool -> AssemblySymbol list)) -> FSharp.Analyzers.SDK.Message []) option =
    None
  ...
  do
    checker.ScriptTypecheckRequirementsChanged.Add
      (fun () ->
  ...
  do
    fileChecked.Publish.Add
      (fun (parseAndCheck, file, version) ->
        async {
          try
            ...
            | None when File.Exists(file) ->
                ...
                let payload =
                  if Utils.isAScript file then
                    BackgroundServices.ScriptFile(file, Ionide.ProjInfo.ProjectSystem.FSIRefs.TFM.NetCore)
                  else
                    BackgroundServices.SourceFile file

                if backgroundServiceEnabled then
                  BackgroundServices.updateFile (payload, ctn |> String.concat "\n", 0)
  ...
  do
    state.ProjectController.Notifications.Add
      (fun ev ->
        match ev with
        | ProjectResponse.Project (p, isFromCache) ->
            if backgroundServiceEnabled then
              ...
              opts
              |> Option.iter (fun opts -> BackgroundServices.updateProject (p.ProjectFileName, opts))
  ...
  member __.SetFileContent(file: SourceFilePath, lines: LineStr [], version, tfmIfScript) =
    ...
    let payload =
      if Utils.isAScript file then
        BackgroundServices.ScriptFile(file, tfmIfScript)
      else
        BackgroundServices.SourceFile file

    if backgroundServiceEnabled then
      BackgroundServices.updateFile (payload, lines |> String.concat "\n", defaultArg version 0)

  member private x.MapResultAsync(successToString: 'a -> Async<CoreResponse<'b>>,
  ...
  member x.DotnetNewRun (templateShortName: string)
                        (name: string option)
  ...
  member _.FsProjMoveFileUp (fsprojPath: string) (file: string) =
    async {
      FsProjEditor.moveFileUp fsprojPath file
      return CoreResponse.Res()
    }

  member _.FsProjMoveFileDown (fsprojPath: string) (file: string) =
    async {
      FsProjEditor.moveFileDown fsprojPath file
      return CoreResponse.Res()
    }

  member _.FsProjAddFileAbove (fsprojPath: string) (file: string) (newFileName: string) =
    async {
      try
        ...
        return CoreResponse.Res()
      with ex -> return CoreResponse.ErrorRes ex.Message
    }

  member _.FsProjAddFileBelow (fsprojPath: string) (file: string) (newFileName: string) =
    async {
      try
        let dir = Path.GetDirectoryName fsprojPath
        let newFilePath = Path.Combine(dir, newFileName)
  ...
  member _.FsProjAddFile (fsprojPath: string) (file: string) =
    async {
      try
        let dir = Path.GetDirectoryName fsprojPath
  ...
  member x.Declarations file lines version =
    async {
      let file = Path.GetFullPath file
  ...
  member x.Completion (tyRes: ParseAndCheckResults)
                      (pos: pos)
                      lineStr
                      (lines: string [])
          ...
          | Some tree ->
              try
                ...
                let! enrichedWarnings =
                  Lint.lintWithConfiguration linterConfiguration ctok tree source tyRes.GetCheckResults
              ...
              with ex ->
                commandsLogger.error (
                  Log.setMessage "error while linting {file}: {message}"
                  >> Log.addContextDestructured "file" file
                  >> Log.addContextDestructured "message" ex.Message
                  >> Log.addExn ex
                )
    ...
    async {
      ...
      | Some (patMatchExpr, unionTypeDefinition, insertionPos) ->
          ...
          if shouldGenerateUnionPatternMatchCases patMatchExpr unionTypeDefinition then
            ...
            let pos =
              mkPos insertionPos.InsertionPos.Line insertionPos.InsertionPos.Column
  ...
  member x.GetAbstractClassStub (tyRes: ParseAndCheckResults)
                                (objExprRange: range)
                                (lines: LineStr [])
                                (lineStr: LineStr)
                                =
  ...
  member x.WorkspaceLoad (files: string list)
  ...
  member x.GetRangesAtPosition file positions =
    ...
    | Some pr ->
        positions
        |> List.map (fun x -> UntypedAstUtils.getRangesAtPosition pr.ParseTree x)
        |> CoreResponse.Res
  ...
  member x.ScopesForFile(file: string) =
    asyncResult {
      ...
      | Some ast' ->
          let ranges =
            Structure.getOutliningRanges sourceLines ast'
  ...
  member x.GetHighlighting(file: SourceFilePath) =
    ...
    async {
      ...
      let res =
        ...
        | Some res ->
            let r =
              res.GetCheckResults.GetSemanticClassification(None)
    ...
    asyncResult {
      ...
      let getGenerics line (token: FSharpTokenInfo) =
        async {
          ...
          match res with
          | Ok tip -> return TipFormatter.extractGenerics tip
          | _ ->
              commandsLogger.info (
      ...
      let getStartingPipe =
        ...
        | x :: y :: xs when
          ...
          && y.TokenName.ToUpper() = "INFIX_BAR_OP" -> Some y
        | _ -> None
