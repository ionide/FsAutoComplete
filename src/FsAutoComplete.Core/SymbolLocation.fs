...
let getDeclarationLocation (symbolUse: FSharpSymbolUse, currentDocument: NamedText, state: State): SymbolDeclarationLocation option =
    if symbolUse.IsPrivateToFile then Some SymbolDeclarationLocation.CurrentDocument
    else
        ...
        | Some loc ->
            ...
            if isScript && filePath = currentDocument.FileName then
                ...
                let projectsThatContainFile =
                  ...
                  |> Seq.choose (fun (_, p) ->
                    if p.SourceFiles |> Array.contains (UMX.untag filePath) then Some p else None
                  )
                  |> Seq.distinct
                  |> Seq.toList
                ...
                | [] -> None
                | projects -> Some (SymbolDeclarationLocation.Projects (projectsThatContainFile @ projects, isSymbolLocalForProject))
        | None -> None
