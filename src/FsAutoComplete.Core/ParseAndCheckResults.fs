...
type ParseAndCheckResults(parseResults: FSharpParseFileResults,
  ...
  member __.TryGetMethodOverrides (lines: LineStr []) (pos: pos) =
    async {
      ...
      let commas, _, _ =
        ...
        let rec prevPos (line, col) =
          ...
          | _, 1 ->
              ...
              if prevLine.Length = 0 then
                prevPos (line - 1, 1)
              else
                line - 1, prevLine.Length
          | _ -> line, col - 1
        ...
        let rec loop commas depth (line, col) =
          if (line, col) <= (1, 1) then
            ...
            if (ch = '(' || ch = '{' || ch = '[') && depth > 0 then
              loop commas (depth - 1) (prevPos (line, col))
            elif ch = ')' || ch = '}' || ch = ']' then
              loop commas (depth + 1) (prevPos (line, col))
            elif ch = '(' || ch = '<' then
              commas, line, col
            else
              loop commas depth (prevPos (line, col))
      ...
      let paramLocations =
        parseResults.FindNoteworthyParamInfoLocations pos
    ...
    async {
      ...
      | Some (col, identIsland) ->
          ...
          let tryRecoverExternalSymbolForNonexistentDecl
            ...
            : Async<ResultOrString<string<LocalPath> * string<NormalizedRepoPathSegment>>> =
            async {
              match Lexer.findLongIdents (pos.Column - 1, lineStr) with
              | None ->
                  return
              ...
              | Some (col, identIsland) ->
                  ...
                  | None ->
                      return
                        ResultOrString.Error(
                          sprintf
                            "Range for nonexistent file found, no symboluse found: %s"
                            rangeInNonexistentFile.FileName
                        )
                  | Some sym ->
                      ...
                      | Some fullFilePath ->
                          return
                            Ok(
                              UMX.tag<LocalPath> fullFilePath,
                              UMX.tag<NormalizedRepoPathSegment> rangeInNonexistentFile.FileName
                            )
                      | None ->
                          return
                            ResultOrString.Error(
                        ...
                        ResultOrString.Ok(
                          FindDeclarationResult.ExternalDeclaration
                            { File = UMX.untag localFilePath
                              Line = rangeInNonexistentFile.StartLine
                              Column = rangeInNonexistentFile.StartColumn }
                        )
                ...
                async {
                  ...
                  | Some fullName ->
                      ...
                      let decompile assembly externalSym =
                        ...
                        | Error (Decompiler.FindExternalDeclarationError.ReferenceHasNoFileName assy) ->
                            ResultOrString.Error(
                              sprintf "External declaration assembly '%s' missing file name" assy.SimpleName
                            )
                        | Error (Decompiler.FindExternalDeclarationError.ReferenceNotFound assy) ->
                            ResultOrString.Error(sprintf "External declaration assembly '%s' not found" assy)
          ...
          | FSharpToolTipText (elems) when
            ...
            |> List.forall ((=) FSharpToolTipElement.None) ->
              ...
              | [ ident ] ->
                  ...
                  | true, tip -> return Ok tip
                  | _ -> return ResultOrString.Error "No tooltip information"
              | _ -> return ResultOrString.Error "No tooltip information"
          | _ -> return Ok(tip)
    }
      ...
      | Some (col, identIsland) ->
          ...
          | FSharpToolTipText (elems) when
            ...
            && symbol.IsNone ->
              ...
              | [ ident ] ->
                  match KeywordList.keywordTooltips.TryGetValue ident with
                  | true, tip -> return Ok(tip, ident, "", None)
      ...
      | Some (col, identIsland) ->
          ...
          | FSharpToolTipText (elems) when
            ...
            && symbol.IsNone ->
              ...
              | [ ident ] ->
                  match KeywordList.keywordTooltips.TryGetValue ident with
                  | true, tip -> return Ok(Some tip, None, (ident, (DocumentationFormatter.emptyTypeTip)), "", "")
                  | _ -> return Error "No tooltip information"
              | _ -> return Error "No documentation information"
          | _ ->
              match symbol with
              | None -> return Error "No documentation information"
              | Some symbol ->
                  ...
                  | Some (signature, footer, cn) ->
                      ...
                      | SymbolUse.TypeAbbreviation symbol ->
                          return
                            Ok(
                              None,
                              Some(
                                symbol.GetAbbriviatedParent().XmlDocSig,
                                symbol.GetAbbriviatedParent().Assembly.FileName
                                |> Option.defaultValue ""
                              ),
                              signature,
                              footer,
                      ...
                      | _ -> return Ok(Some tip, None, signature, footer, cn)
    }
  ...
  member __.TryGetSymbolUse (pos: pos) (lineStr: LineStr): Async<FSharpSymbolUse option> =
    async {
      match Lexer.findLongIdents (pos.Column - 1, lineStr) with
      | None -> return None
      ...
      | Some (colu, identIsland) ->
          ...
          | Some symboluse ->
              ...
              | :? FSharpMemberOrFunctionOrValue as symbol ->
                  let typ =
                    symbol.ReturnParameter.Type.Format symboluse.DisplayContext
                  ...
                  if symbol.IsPropertyGetterMethod then
                    ...
                    let parms =
                      ...
                      |> Seq.map (
                        Seq.map (fun p -> p.DisplayName, p.Type.Format symboluse.DisplayContext)
                        >> Seq.toList
                      )
                      |> Seq.toList
                    ...
                    | [ [] ] when
                      symbol.IsMember
                      && (not symbol.IsPropertyGetterMethod) -> return Ok(typ, [ [ ("unit", "unit") ] ], [])
                    | _ -> return Ok(typ, parms, generics)
              | :? FSharpField as symbol ->
                  let typ =
                    symbol.FieldType.Format symboluse.DisplayContext

                  return Ok(typ, [], [])
              | _ -> return ResultOrString.Error "Not a member, function or value"
    }
  ...
  member __.TryGetCompletions (pos: pos) (lineStr: LineStr) filter (getAllSymbols: unit -> AssemblySymbol list) =
    async {
      try
        let longName =
          FSharp.Compiler.QuickParse.GetPartialLongNameEx(lineStr, pos.Column - 2)
