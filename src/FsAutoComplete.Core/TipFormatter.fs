        ...
        |> Array.map
             (fun line ->
               if not (String.IsNullOrWhiteSpace line)
                  && line.StartsWith(tabsOffset) then
    ...
    | true when
      ...
      && reader.NodeType = XmlNodeType.Element ->
        try
          ...
          |> Some
        with ex -> indentationSize, Some acc
  ...
  if xmlDocCache.ContainsKey xmlFile then
    ...
    | Some actualXmlFile ->
        ...
        try
          ...
          Some xmlDoc
        with ex -> None // TODO: Remove the empty map from cache to try again in the next request?
  ...
  |> List.choose
       ...
       | FSharpToolTipElement.Group items ->
           ...
           Some(
             ...
             |> List.map
                  (fun it ->
                    (it.MainDescription + getRemarks it, buildFormatComment it.XmlDoc FormatCommentStyle.Legacy None))
           )
