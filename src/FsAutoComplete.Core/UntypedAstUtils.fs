  ...
  let getHashLoadDirectiveResolvedPathAtPosition (pos: pos) (ast: ParsedInput): string option =
    ...
    |> Array.tryPick
         ...
         | Load (ExistingFile f, range) when
           ...
           rangeContainsPos range pos -> Some f
         | _ -> None)
...
module Outlining =
  ...
  module private Range =
    ...
    let inline modBoth (r: range) modStart modEnd =
      let rStart =
        Range.mkPos r.StartLine (r.StartColumn + modStart)

      let rEnd =
        Range.mkPos r.EndLine (r.EndColumn - modEnd)
  ...
  let rec private parseExpr expression =
    seq {
      match expression with // prevent double collapsing on elifs
      ...
      | SynExpr.DoBang (e, r) ->
          yield!
            rcheck Scope.Do Collapse.Below
            <| Range.modStart r 3
      ...
      | SynExpr.LetOrUseBang (_, _, _, pat, e1, ands, e2, _) ->
          ...
          yield!
            rcheck Scope.LetOrUseBang Collapse.Below
            <| Range.endToEnd pat.Range e1.Range
      ...
      | SynExpr.Match (debugPointAtBinding, _, clauses, r) ->
          ...
          | DebugPointAtBinding pr ->
              yield!
                rcheck Scope.Match Collapse.Below
                <| Range.endToEnd pr r
          | _ -> ()
      ...
      | SynExpr.MatchLambda (_, _, clauses, _, r) ->
          yield!
            rcheck Scope.MatchLambda Collapse.Below
            <| Range.modStart r 8
      ...
      | SynExpr.App (atomicFlag, isInfix, funcExpr, argExpr, r) ->
          ...
          if ExprAtomicFlag.NonAtomic = atomicFlag
             ...
             && (function
                | SynExpr.CompExpr _ -> false
                | _ -> true)
                  argExpr then
            yield!
              rcheck Scope.SpecialFunc Collapse.Below
              <| Range.endToEnd funcExpr.Range r
      ...
      | SynExpr.ArrayOrListOfSeqExpr (isArray, e, r) ->
          yield!
            rcheck Scope.ArrayOrList Collapse.Same
            <| Range.modBoth r (if isArray then 2 else 1) (if isArray then 2 else 1)
      ...
      | SynExpr.CompExpr (arrayOrList, _, e, r) ->
          if arrayOrList then
            ...
            yield!
              rcheck Scope.CompExpr Collapse.Same
              <| Range.modBoth r 1 1

          yield! parseExpr e
      | SynExpr.ObjExpr (_, _, bindings, _, newRange, wholeRange) ->
          let r =
            mkFileIndexRange newRange.FileIndex newRange.End (Range.mkPos wholeRange.EndLine (wholeRange.EndColumn - 1))
      ...
      | SynExpr.TryWith (e, _, matchClauses, tryRange, withRange, tryPoint, withPoint) ->
          ...
          | DebugPointAtTry.Yes r ->
              yield!
                rcheck Scope.TryWith Collapse.Below
                <| Range.endToEnd r tryRange
          | _ -> ()

          match withPoint with
          | DebugPointAtWith.Yes r ->
              yield!
                rcheck Scope.WithInTryWith Collapse.Below
                <| Range.endToEnd r withRange
          | _ -> ()
      ...
      | SynExpr.TryFinally (tryExpr, finallyExpr, r, tryPoint, finallyPoint) ->
          ...
          | DebugPointAtTry.Yes tryRange ->
              yield!
                rcheck Scope.TryFinally Collapse.Below
                <| Range.endToEnd tryRange r
          | _ -> ()

          match finallyPoint with
          | DebugPointAtFinally.Yes finallyRange ->
              yield!
                rcheck Scope.FinallyInTryFinally Collapse.Below
                <| Range.endToEnd finallyRange r
          | _ -> ()
      ...
      | SynExpr.IfThenElse (e1, e2, e3, seqPointInfo, _, _, r) ->
          ...
          | DebugPointAtBinding rt ->
              yield!
                rcheck Scope.ThenInIfThenElse Collapse.Below
                <| Range.endToEnd rt e2.Range
          | _ -> ()

          yield! parseExpr e1
          yield! parseExpr e2

          match e3 with // prevent double collapsing on elifs
          | Some e ->
              match e with // prevent double collapsing on elifs
              | SynExpr.IfThenElse (_, _, _, _, _, _, _) -> yield! parseExpr e
              | _ ->
      ...
      | SynExpr.Lambda (_, _, pats, e, _, r) ->
          ...
          | SynSimplePats.Typed (_, _, pr) ->
              yield!
                rcheck Scope.Lambda Collapse.Below
                <| Range.endToEnd pr r
      ...
      | SynExpr.Quote (_, isRaw, e, _, r) ->
          ...
          yield!
            rcheck Scope.Quote Collapse.Same
            <| Range.modBoth r (if isRaw then 3 else 2) (if isRaw then 3 else 2)

          yield! parseExpr e
      | SynExpr.Tuple (_, es, _, r) ->
          yield! rcheck Scope.Tuple Collapse.Same r
          yield! Seq.collect parseExpr es
      | SynExpr.Paren (e, _, _, _) -> yield! parseExpr e
      | SynExpr.Record (recCtor, recCopy, recordFields, r) ->
          ...
          yield!
            recordFields
            |> (Seq.choose (fun (_, e, _) -> e)
                >> Seq.collect parseExpr)
          // exclude the opening `{` and closing `}` of the record from collapsing
          yield!
            rcheck Scope.Record Collapse.Same
            <| Range.modBoth r 1 1
      | _ -> ()
    }
  ...
  and private parseMatchClause (SynMatchClause.Clause (synPat, _, e, _, _)) =
    seq {
      yield!
        rcheck Scope.MatchClause Collapse.Same
        <| Range.startToEnd synPat.Range e.Range // Collapse the scope after `->`
    ...
    seq {
      let attrListRange =
        if List.isEmpty attrs then
          ...
          rcheck Scope.Attribute Collapse.Same
          <| Range.startToEnd (attrs.[0].Range) (attrs.[attrs.Length - 1].ArgExpr.Range)
      ...
      | hd :: tl ->
          ...
          yield!
            ...
            |> Seq.collect
                 (fun attr ->
                   rcheck Scope.Attribute Collapse.Same
                   <| Range.startToEnd attr.Range attr.ArgExpr.Range)
          // visit the expressions inside each attribute
          yield!
            attrs
            |> Seq.collect (fun attr -> parseExpr attr.ArgExpr)
    }
  ...
  and private parseBinding (Binding (_, kind, _, _, AllAttrs attrs, _, _, _, _, e, br, _) as b) =
    seq {
      ...
      | SynBindingKind.NormalBinding ->
          yield!
            rcheck Scope.LetOrUse Collapse.Below
            <| Range.endToEnd b.RangeOfBindingSansRhs b.RangeOfBindingAndRhs
  ...
  and private parseSynMemberDefn d =
    seq {
      ...
      | SynMemberDefn.Interface (tp, iMembers, _) ->
          yield!
            rcheck Scope.Interface Collapse.Below
            <| Range.endToEnd tp.Range d.Range

          match iMembers with
          | Some members -> yield! Seq.collect parseSynMemberDefn members
          | None -> ()
      | SynMemberDefn.NestedType (td, _, _) -> yield! parseTypeDefn td
      | SynMemberDefn.AbstractSlot (ValSpfn (_, _, _, synt, _, _, _, _, _, _, _), _, r) ->
          yield!
            rcheck Scope.Member Collapse.Below
            <| Range.startToEnd synt.Range r
    ...
    seq {
      ...
      | SynTypeDefnSimpleRepr.Enum (cases, er) ->
          ...
          yield!
            cases
            |> Seq.collect
                 (fun (SynEnumCase.EnumCase (AllAttrs attrs, _, _, _, cr)) ->
                   seq {
                     yield! rcheck Scope.EnumCase Collapse.Below cr
                     yield! parseAttributes attrs
                   })
      | SynTypeDefnSimpleRepr.Record (_opt, fields, rr) ->
          ...
          yield!
            fields
            |> Seq.collect
                 (fun (SynField.Field (AllAttrs attrs, _, _, _, _, _, _, fr)) ->
                   seq {
                     yield! rcheck Scope.RecordField Collapse.Below fr
                     yield! parseAttributes attrs
                   })
      | SynTypeDefnSimpleRepr.Union (_opt, cases, ur) ->
          ...
          yield!
            cases
            |> Seq.collect
                 (fun (SynUnionCase.UnionCase (AllAttrs attrs, _, _, _, _, cr)) ->
                   seq {
                     yield! rcheck Scope.UnionCase Collapse.Below cr
                     yield! parseAttributes attrs
                   })
      | _ -> ()
    }
      ...
      | SynTypeDefnRepr.ObjectModel (defnKind, objMembers, _) ->
          ...
          | SynTypeDefnKind.TyconAugmentation ->
              yield!
                ...
                <| Range.endToEnd componentInfo.Range range
          | _ ->
              yield!
                rcheck Scope.Type Collapse.Below
                <| Range.endToEnd componentInfo.Range range
  ...
  let private getConsecutiveModuleDecls
    (predicate: SynModuleDecl -> range option)
    (scope: Scope)
    ...
    let selectRanges (ranges: range list) =
      ...
      | [ r ] ->
          Some
          <| ScopeRange(scope, Collapse.Same, (Range.mkRange "" r.Start r.End))
      | lastRange :: rest ->
          let firstRange = Seq.last rest

          Some
          <| ScopeRange(scope, Collapse.Same, (Range.mkRange "" firstRange.Start lastRange.End))
  ...
  let collectHashDirectives =
    getConsecutiveModuleDecls
      ...
      | SynModuleDecl.HashDirective (ParsedHashDirective (directive, _, _), r) ->
          let prefixLength =
            "#".Length + directive.Length + " ".Length
  ...
  let rec private parseDeclaration (decl: SynModuleDecl) =
    seq {
      match decl with
      | SynModuleDecl.Let (_, bindings, _) -> yield! parseBindings bindings
      | SynModuleDecl.Types (types, _) -> yield! Seq.collect parseTypeDefn types
      // Fold the attributes above a module
      | SynModuleDecl.NestedModule (SynComponentInfo.ComponentInfo (AllAttrs attrs, _, _, _, _, _, _, cmpRange),
                                    _,
                                    decls,
                                    _,
                                    _) ->
          ...
          yield!
            rcheck Scope.Module Collapse.Below
            <| Range.endToEnd cmpRange decl.Range
      ...
      | SynModuleDecl.DoExpr (_, e, _) -> yield! parseExpr e
      | SynModuleDecl.Attributes (AllAttrs attrs, _) -> yield! parseAttributes attrs
      | _ -> ()
    }

  let private parseModuleOrNamespace moduleOrNs =
    seq {
      let (SynModuleOrNamespace.SynModuleOrNamespace (_, _, _, decls, _, _, _, _)) = moduleOrNs
  ...
  let getCommentRanges (lines: string []) =
    let comments: CommentList list =
      lines
      |> Array.foldi
           (fun ((lastLineNum, currentComment: CommentList option, result) as state) lineNum lineStr ->
             match lineStr.TrimStart(), currentComment with
             | Comment commentType, Some comment ->
                 if comment.Type = commentType
                    && lineNum = lastLineNum + 1 then
                   comment.Lines.Add(lineNum, lineStr)
                   lineNum, currentComment, result
                 else
                   lineNum, Some(CommentList.New commentType (lineNum, lineStr)), comment :: result
             | Comment commentType, None -> lineNum, Some(CommentList.New commentType (lineNum, lineStr)), result
             | _, Some comment -> lineNum, None, comment :: result
             | _ -> state)
           (-1, None, [])
    ...
    |> List.map
         (fun comment ->
           ...
           ScopeRange(
             scopeType,
             Collapse.Same,
             Range.mkRange "" (Range.mkPos (startLine + 1) startCol) (Range.mkPos (endLine + 1) endCol)
           ))
