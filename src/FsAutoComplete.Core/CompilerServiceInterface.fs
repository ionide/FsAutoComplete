...
type FSharpCompilerServiceChecker(backgroundServiceEnabled) =
  ...
  member self.GetProjectOptionsFromScript(file, source, tfm) =
    async {
      ...
      try
        ...
        | None ->
            logQueueLength
              optsLogger
              (Log.setMessage "{file} is not a FAKE script"
               >> Log.addContextDestructured "file" file)
        ...
        | Some (detectionInfo) ->
            logQueueLength
              optsLogger
              (Log.setMessage "{file} is a FAKE script"
               >> Log.addContextDestructured "file" file)

            try
              let otherOpts =
                FakeSupport.getProjectOptions detectionInfo

              logQueueLength
                optsLogger
                (Log.setMessage "Discovered FAKE options {otherOpts} "
                 >> Log.addContextDestructured "file" file
                 >> Log.addContextDestructured "otherOpts" otherOpts)
            ...
            with e ->
              logQueueLength
                optsLogger
                (Log.setLogLevel LogLevel.Error
                 >> Log.setMessage "Error in FAKE script support"
                 >> Log.addExn e)
      ...
      with e ->
        logQueueLength
          optsLogger
          (Log.setMessage "error while checking if {file} is a FAKE script"
           >> Log.addContextDestructured "file" file
           >> Log.addExn e)
  ...
  member __.FileChecked = checker.FileChecked
  ...
  member __.ParseAndCheckFileInProject(filePath, version, source, options) =
    async {
      let opName =
        sprintf "ParseAndCheckFileInProject - %s" filePath
      ...
      with ex -> return ResultOrString.Error(ex.ToString())
    }

  member __.TryGetRecentCheckResultsForFile(file, options, ?source) =
    let opName =
      sprintf "TryGetRecentCheckResultsForFile - %s" file
  ...
  member x.GetUsesOfSymbol(file, options: (SourceFilePath * FSharpProjectOptions) seq, symbol: FSharpSymbol) =
    async {
      logQueueLength
        checkerLogger
  ...
  member __.GetDeclarations(fileName, source, options, version) =
    async {
      logQueueLength
        checkerLogger
