...
module SymbolUse =
          ...
          |> List.collect
               (fun cleanFullName ->
                 if ent.IsAttributeType then
                   [ cleanFullName
                     cleanFullName.[0..cleanFullName.Length - attributeSuffixLength - 1] ]
                 else
                   [ cleanFullName ])
  ...
  let (|Property|_|) =
    ...
    | MemberFunctionOrValue symbol when
      ...
      || symbol.IsPropertySetterMethod -> Some symbol
    | _ -> None
...
module SymbolPatterns =
  ...
  let (|RefCell|_|) (ty: FSharpType) =
    ...
    | TypeWithDefinition def when
      def.IsFSharpRecord
      && def.FullName = "Microsoft.FSharp.Core.FSharpRef`1" -> Some()
    | _ -> None
