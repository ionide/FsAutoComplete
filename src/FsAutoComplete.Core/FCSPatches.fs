...
module SyntaxTraversal =
  ...
  type SyntaxWalkerBase<'T>() =
    abstract VisitExpr: path: SyntaxVisitorPath * traverseSynExpr: (SynExpr -> 'T []) * defaultTraverse: (SynExpr -> 'T []) * synExpr: SynExpr -> 'T []
    default _.VisitExpr(path: SyntaxVisitorPath, traverseSynExpr: SynExpr -> 'T [], defaultTraverse: SynExpr -> 'T [], synExpr: SynExpr) =
    ...
    abstract VisitImplicitInherit: path: SyntaxVisitorPath * defaultTraverse: (SynExpr -> 'T []) * inheritedType: SynType * synArgs: SynExpr * range: range -> 'T []
    ...
    abstract VisitModuleDecl: path: SyntaxVisitorPath * defaultTraverse: (SynModuleDecl -> 'T []) * synModuleDecl: SynModuleDecl -> 'T []
    ...
    abstract VisitBinding: path: SyntaxVisitorPath * defaultTraverse: (SynBinding -> 'T []) * synBinding: SynBinding -> 'T []
    ...
    abstract VisitMatchClause: path: SyntaxVisitorPath * defaultTraverse: (SynMatchClause -> 'T []) * matchClause: SynMatchClause -> 'T []
    ...
    abstract VisitInheritSynMemberDefn: path: SyntaxVisitorPath * componentInfo: SynComponentInfo * typeDefnKind: SynTypeDefnKind * SynType  * SynMemberDefns * range -> 'T []
    ...
    abstract VisitRecordField: path: SyntaxVisitorPath * copyOpt: SynExpr option * recordField: LongIdentWithDots option -> 'T []
    ...
    abstract VisitLetOrUse: path: SyntaxVisitorPath * isRecursive: bool * defaultTraverse: (SynBinding -> 'T []) * bindings: SynBinding list * range: range -> 'T []
  ...
  let Walk(parseTree, walker: SyntaxWalkerBase<'T>) =
        let rec traverseSynModuleDecl origPath (decl:SynModuleDecl) =
            let defaultTraverse m =
                ...
                | SynModuleDecl.Types(synTypeDefnList, _range) -> synTypeDefnList |> List.toArray |> Array.collect (traverseSynTypeDefn path)
                ...
                | SynModuleDecl.HashDirective(parsedHashDirective, range) -> walker.VisitHashDirective (path, parsedHashDirective, range)
        ...
        and traverseSynModuleOrNamespace origPath (SynModuleOrNamespace(_longIdent, _isRec, _isModule, synModuleDecls, _preXmlDoc, _synAttributes, _synAccessOpt, _range) as mors) =
        ...
        and traverseSynExpr origPath (expr:SynExpr) =
            let defaultTraverse e =
                ...
                | SynExpr.Quote (_, _, quotedExpression , _, _range) ->
                    traverseSynExpr quotedExpression
                ...
                | SynExpr.InterpolatedString (parts, _, _) ->
                    [| for part in parts do
                          ...
                          | SynInterpolatedStringPart.FillExpr (fillExpr, _) ->
                              yield! traverseSynExpr fillExpr |]
                ...
                | SynExpr.ArrayOrList (_, synExprList, _range) ->
                    synExprList |> List.toArray |> Array.collect traverseSynExpr
                ...
                | SynExpr.ObjExpr (ty,baseCallOpt, _,binds, members, ifaces,_range1,_range2) ->
                    [|
                        yield! ifaces
                            |> Seq.collect (fun (SynInterfaceImpl(interfaceTy = ty)) -> walker.VisitInterfaceSynMemberDefnType(path, ty))
                        match baseCallOpt with
                        | Some(expr,_) ->
                            // this is like a call to 'new', so mock up a 'new' so we can recurse and use that existing logic
                            let newCall = SynExpr.New (false, ty, expr, FSharp.Compiler.Text.Range.unionRanges ty.Range expr.Range)
                ...
                | SynExpr.ComputationExpr (_, synExpr, _range) ->
                    ...
                    [| yield!
                        ...
                        | false, SynExpr.Ident ident -> walker.VisitRecordField(path, None, Some (LongIdentWithDots([ident], [])))
                ...
                | SynExpr.Lambda (args = args; body = body) ->
                    [| match args with
                       | SynSimplePats.SimplePats(pats,_) ->
                         yield! walker.VisitSimplePats(path, pats)
                       | _ -> ()
                ...
                | SynExpr.IndexRange (leftExpr, _, rightExpr, _, _, _) ->
                    [| match leftExpr with Some e -> yield! traverseSynExpr e | None -> ()
                       match rightExpr with Some e -> yield! traverseSynExpr e | None -> () |]
            ...
            let defaultTraverse p =
                ...
                | SynPat.LongIdent(argPats = argPats) ->
                    ...
                    | SynArgPats.NamePatPairs (ps, _) ->
                        ps |> List.map (fun (_,_,pat) -> pat) |> List.toArray |> Array.collect (traversePat path)
                | SynPat.Typed (p, ty, _) ->
                    [| yield! traversePat path p; yield! traverseSynType path ty |]
                | _ -> [| |]
        ...
        and normalizeMembersToDealWithPeculiaritiesOfGettersAndSetters path traverseInherit (synMemberDefns:SynMemberDefns) =
            synMemberDefns
                    ...
                    |> Seq.collect (fun (_, mems) ->
                        ...
                        |  [SynMemberDefn.Member(memberDefn = SynBinding(headPat = SynPat.LongIdent(longDotId = lid1; propertyKeyword = Some(info1); extraId = Some getset1))) as mem1
                            SynMemberDefn.Member(memberDefn = SynBinding(headPat = SynPat.LongIdent(longDotId = lid2; propertyKeyword = Some(info2); extraId = Some getset2))) as mem2] -> // can happen if one is a getter and one is a setter
                            // ensure same long id
                            assert( (lid1.Lid,lid2.Lid) ||> List.forall2 (fun x y -> x.idText = y.idText) )
                            // ensure one is getter, other is setter
                            assert( (getset1.idText="set" && getset2.idText="get") ||
        ...
        and traverseSynTypeDefn origPath (SynTypeDefn(synComponentInfo, synTypeDefnRepr, synMemberDefns, implicitCtor, _tRange, _) as tydef) =
                ...
                [|
                    ...
                    | SynTypeDefnRepr.Simple(synTypeDefnSimpleRepr, _range) ->
                        ...
                        | SynTypeDefnSimpleRepr.TypeAbbrev(_,synType,m) ->
                            yield! walker.VisitTypeAbbrev(path, synType, m)
                        | _ ->
                            () // enums/DUs/record definitions don't have any SynExprs inside them
                    yield! implicitCtor |> Option.toArray |> Array.collect (traverseSynMemberDefn path (fun _ -> [| |]))
                    yield! synMemberDefns |> normalizeMembersToDealWithPeculiaritiesOfGettersAndSetters path (fun _ -> [| |])
                |]
            | nodes -> nodes
        ...
        and traverseSynMemberDefn path traverseInherit (m:SynMemberDefn)  =
            ...
            | SynMemberDefn.Interface(interfaceType = interfaceType; members = members) ->
                ...
                | [| |] ->
                    match members with
                    | None -> [| |]
                    | Some(x) -> [| yield! x |> normalizeMembersToDealWithPeculiaritiesOfGettersAndSetters path (fun _ -> [| |]) |]
                | ok -> ok
...
module ParsedInputExtensions =
  ...
  type FSharp.Compiler.Syntax.ParsedInput with
      member parseTree.GetSemanticClassification(_: range option): SemanticClassificationItem [] =
        SyntaxTraversal.Walk(parseTree, walker)
