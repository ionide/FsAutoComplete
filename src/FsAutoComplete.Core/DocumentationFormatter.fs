...
module DocumentationFormatter =
  ...
  let rec formatType displayContext (typ: FSharpType): (string * int) list =
    ...
    if typ.IsTupleType || typ.IsStructTupleType then
      ...
      let t =
        Regex.Replace(org, """(.*?) list""", "List<")

      let t =
        Regex.Replace(t, """(.*?) option""", "Option<")

      let t =
        Regex.Replace(t, """(.*?) lazy""", "Lazy<")

      let t =
        Regex.Replace(t, """(.*?) seq""", "Seq<")

      let t =
        Regex.Replace(t, """(.*?) ?\[\]""", "Array<")
  ...
  let getFuncSignatureWithIdent displayContext (func: FSharpMemberOrFunctionOrValue) (ident: int) =
    ...
    let functionName =
      let name =
        if func.IsConstructor then
          ...
          PrettyNaming.QuoteIdentifierIfNeeded func.LogicalName
        else
          PrettyNaming.QuoteIdentifierIfNeeded func.DisplayName
      ...
      let allLengths =
        ...
        |> List.map
             (fun p ->
               ...
               let normalisedName =
                 PrettyNaming.QuoteIdentifierIfNeeded name
    ...
    | [ [ p ] ] when
      maybeGetter
      && formatParameter displayContext p |> fst = "unit" -> //Member or property with only getter
        modifiers ++ functionName + ":" ++ retType
    | many ->
  ...
  let getEntitySignature displayContext (fse: FSharpEntity) =
               ...
               | None ->
                   [ for f in v do
                       ...
                       yield!
                         f.Overloads false
                         |> Option.map (
                           Seq.map (fun f -> getFuncSignatureForTypeSignature displayContext f false false)
                         )
                         |> Option.defaultValue Seq.empty ])
        |> Seq.toArray
    ...
    let typeDisplay =
      let name =
        let normalisedName =
          PrettyNaming.QuoteIdentifierIfNeeded fse.DisplayName
