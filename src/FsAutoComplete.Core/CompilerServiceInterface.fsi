...
type FSharpCompilerServiceChecker =
  ...
  member GetProjectOptionsFromScript:
    file: string<LocalPath> * source: ISourceTextNew * tfm: FSIRefs.TFM -> Async<FSharpProjectSnapshot * FSharpDiagnostic list>
  ...
  /// <returns></returns>
  member ParseFile:
    filePath: string<LocalPath> * source: ISourceText * snapshot: FSharpProjectSnapshot -> Async<FSharpParseFileResults>
  ...
  member TryGetRecentCheckResultsForFile:
    file: string<LocalPath> * snapshot: FSharpProjectSnapshot * source: ISourceText -> Async<ParseAndCheckResults option>
