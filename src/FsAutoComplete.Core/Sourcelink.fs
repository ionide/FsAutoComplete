...
let private tryGetUrlWithWildcard
  ...
  | m when not m.Success ->
      logger.info (
        Log.setMessage "document {doc} did not match pettern {pattern}"
  ...
  | m ->
      ...
      logger.info (
        Log.setMessage "document {doc} did match pettern {pattern} with value {replacement}"
        >> Log.addContext "doc" document.Name
        >> Log.addContext "pattern" pattern
...
let private downloadFileToTempDir
  ...
  async {
    ...
    do!
      response.ResponseStream.CopyToAsync fileStream
      |> Async.AwaitTask
