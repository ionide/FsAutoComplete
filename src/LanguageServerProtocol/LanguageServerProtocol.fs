      ...
      memorise
        (fun t ->
          if not (FSharpType.IsUnion t) then
            ...
            t
              .BaseType
              .GetCustomAttributes(typedefof<ErasedUnionAttribute>, false)
              .Length > 0)
...
type LspServer() =
  ...
  abstract member TextDocumentDocumentHighlight: TextDocumentPositionParams
...
module Server =
  ...
  let requestHandling<'param, 'result, 'server when 'server :> LspServer>
    (run: 'server -> 'param -> AsyncLspResult<'result>)
    : RequestHandling<'server> =
  ...
  let handleRequest<'server when 'server :> LspServer>
    (requestHandlings: Map<string, RequestHandling<'server>>)
    (request: JsonRpc.Request)
    (lspServer: 'server)
    : Async<JsonRpc.Response option> =
    async {
  ...
  let start<'a, 'b when 'a :> LspClient and 'b :> LspServer>
    (requestHandlings: Map<string, RequestHandling<'b>>)
    (input: Stream)
    (output: Stream)
    (clientCreator: ClientNotificationSender -> 'a)
    (serverCreator: 'a -> 'b)
    =
    let sender =
    ...
    let sendServerRequest (rpcMethod: string) (requestObj: obj): AsyncLspResult<unit> =
      ...
      let req =
        JsonRpc.Request.Create(rpcMethod, serializedResponse)
    ...
    let handleClientRequest (requestString: string): RequestHandlingResult =
      let request =
        JsonConvert.DeserializeObject<JsonRpc.Request>(requestString, jsonSettings)
  ...
  type Client(exec: string, args: string, notificationHandlings: Map<string, NotificationHandler>) =
    ...
    let handleRequest (request: JsonRpc.Request) =
      async {
        ...
        | None ->
            match methodCallResult with
            | Some ok -> return Some(JsonRpc.Response.Success(None, Some ok))
            | None -> return None
      }
