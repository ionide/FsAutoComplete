...
let private tryRangeOfParenEnclosingOpEqualsGreaterUsage input pos =
    ...
    let (|InfixAppOfOpEqualsGreater|_|) =
      function | SynExpr.App(ExprAtomicFlag.NonAtomic, false, SynExpr.App(ExprAtomicFlag.NonAtomic, true, Ident "op_EqualsGreater", actualParamListExpr, range), actualLambdaBodyExpr, _) ->
                    let opEnd = range.End
    ...
    SyntaxTraversal.Traverse(pos, input, { new SyntaxVisitorBase<_>() with
        member _.VisitExpr(_, _, defaultTraverse, expr) =
            match expr with
            | SynExpr.Paren(InfixAppOfOpEqualsGreater(argsRange, opRange), _, _, _) ->
                Some (argsRange, opRange)
              ...
              [ { Title = title
                  ...
                  Edits =
                    [|
                      ...
                      {
                        Range = { Start = fcsPosToLsp argsRange.Start; End = fcsPosToLsp argsRange.Start }
                        NewText = "fun "
                      }
                      // replace `=>` with `->`
                      {
