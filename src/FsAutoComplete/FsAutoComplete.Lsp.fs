...
type FSharpLspClient(sendServerRequest: ClientNotificationSender) =
  ...
  override __.WindowShowMessage(p) =
    sendServerRequest "window/showMessage" (box p)
    |> Async.Ignore

  override __.WindowLogMessage(p) =
    sendServerRequest "window/logMessage" (box p)
    |> Async.Ignore

  override __.TextDocumentPublishDiagnostics(p) =
    sendServerRequest "textDocument/publishDiagnostics" (box p)
    |> Async.Ignore

  ///Custom notification for workspace/solution/project loading events
  member __.NotifyWorkspace(p: PlainNotification) =
    sendServerRequest "fsharp/notifyWorkspace" (box p)
    |> Async.Ignore

  ///Custom notification for initial workspace peek
  member __.NotifyWorkspacePeek(p: PlainNotification) =
    sendServerRequest "fsharp/notifyWorkspacePeek" (box p)
    |> Async.Ignore

  member __.NotifyCancelledRequest(p: PlainNotification) =
    sendServerRequest "fsharp/notifyCancel" (box p)
    |> Async.Ignore

  member __.NotifyFileParsed(p: PlainNotification) =
    sendServerRequest "fsharp/fileParsed" (box p)
    |> Async.Ignore
...
type FsharpLspServer(commands: Commands, lspClient: FSharpLspClient) =
  ...
  let updateConfig (newConfig: FSharpConfig) =
    ...
    | paths ->
        ...
        Loggers.analyzers.info (
          Log.setMessage "{count} Analyzers registered overall"
          >> Log.addContextDestructured "count" total
        )
        ...
        | Some contentChange, Some version ->
            if contentChange.Range.IsNone
              ...
              if config.Linter then
                do! (commands.Lint filePath |> Async.Ignore)
      ...
      (fun n ->
        try
          ...
          | NotificationEvent.Lint (file, warnings) ->
              ...
              diagnosticCollections.AddOrUpdate((uri, "F# Linter"), diags, (fun _ _ -> diags))
              |> ignore
  ...
  member x.positionHandler<'a, 'b when 'b :> ITextDocumentPositionParams> (f: 'b -> FcsRange.pos -> ParseAndCheckResults -> string -> string [] -> AsyncLspResult<'a>)
                                                                          (arg: 'b)
                                                                          : AsyncLspResult<'a> =
    async {
  ...
  member x.positionHandlerWithLatest<'a, 'b when 'b :> ITextDocumentPositionParams> (f: 'b -> FcsRange.pos -> ParseAndCheckResults -> string -> string [] -> AsyncLspResult<'a>)
                                                                                    (arg: 'b)
                                                                                    : AsyncLspResult<'a> =
    async {
      let pos = arg.GetFcsPos()
  ...
  member x.fileHandler<'a> (f: SourceFilePath -> ParseAndCheckResults -> string [] -> AsyncLspResult<'a>)
                           (file: SourceFilePath)
                           : AsyncLspResult<'a> =
    async {
        ...
        fun p ->
          [| ifEnabled (fun _ -> config.UnusedOpensAnalyzer) Fixes.unusedOpens
             ifEnabled
               ...
               (Fixes.resolveNamespace tryGetParseResultsForFile commands.GetNamespaceSuggestions)
             ...
             ifEnabled
               ...
               (Fixes.generateAbstractClassStub
                 tryGetParseResultsForFile
    ...
    asyncResult {
      ...
      if (lineStr.StartsWith "#"
        ...
        match! commands.Completion
                 ...
                 (config.ExternalAutocomplete) with
  ...
  override x.TextDocumentSignatureHelp(sigHelpParams: SignatureHelpParams) =
    ...
    |> x.positionHandlerWithLatest
         (fun p fcsPos tyRes lineStr lines ->
           asyncResult {
             let! (methods, commas) =
               commands.Methods tyRes fcsPos lines
             ...
             let sigs =
               ...
               |> Array.map
                    (fun m ->
                      let (sign, comm) =
                        TipFormatter.formatTip m.Description
                        |> List.head
                        |> List.head
             ...
             let activSig =
               let sigs =
                 sigs
                 |> Seq.sortBy (fun n -> n.Parameters.Value.Length)

               sigs
               |> Seq.findIndex (fun s -> s.Parameters.Value.Length >= commas)
               |> fun index ->
                    if index + 1 >= (sigs |> Seq.length) then
                      index
                    else
                      index + 1
    ...
    async {
      ...
      let res =
        ...
        | CoreResponse.Res decls ->
            ...
            |> Array.map (
              fst
              >> getSymbolInformations p.TextDocument.Uri glyphToSymbolKind
            )
            |> Seq.collect id
            |> Seq.toArray
            |> Some
  ...
  override __.CodeLensResolve(p) =
    ...
    let handler f (arg: CodeLens) =
      async {
        let pos =
          FcsRange.mkPos (arg.Range.Start.Line + 1) (arg.Range.Start.Character + 2)
  ...
  member x.FSharpSignatureData(p: TextDocumentPositionParams) =
    ...
    let handler f (arg: TextDocumentPositionParams) =
      async {
        let pos =
          FcsRange.mkPos (p.Position.Line) (p.Position.Character + 2)
        ...
        return!
          ...
          | ResultOrString.Ok (options, _, lineStr) ->
              ...
              with e -> AsyncLspResult.internalError e.Message
      }
           ...
           async {
             ...
             let res =
               ...
               | CoreResponse.Res (typ, parms, generics) ->
                   { Content =
                       CommandResponse.signatureData FsAutoComplete.JsonSerializer.writeJson (typ, parms, generics) }
                   |> success
  ...
  member __.FSharpWorkspaceLoad(p) =
    async {
      logger.info (
  ...
  member __.FakeTargets(p: FakeTargetsRequest) =
    async {
      logger.info (
        Log.setMessage "FakeTargets Request: {parms}"
        >> Log.addContextDestructured "parms" p
      )
  ...
  member __.FakeRuntimePath(p) =
    async {
      logger.info (
        Log.setMessage "FakeRuntimePath Request: {parms}"
        >> Log.addContextDestructured "parms" p
      )
  ...
  member __.GetHighlighting(p: HighlightingRequest) =
    async {
      logger.info (
        Log.setMessage "GetHighlighting Request: {parms}"
        >> Log.addContextDestructured "parms" p
      )
...
let startCore (commands: Commands) =
  ...
  LanguageServerProtocol.Server.start
    ...
    output
    FSharpLspClient
    (fun lspClient -> FsharpLspServer(commands, lspClient))
